    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
            ...
            if (redisUtil.isBlackList(authToken)) {
                ...
                response.getWriter().write(JSON.toJSONString(Result.error(Constants.CODE_401,"认证凭证已列入黑名单")));
                return;
            }
            ...
            if (redisUtil.hasKey(authToken)) {//判断redis是否有保存
                ...
                if (JwtTokenUtil.isExpiration(expirationTime)) {
                    ...
                    if (DateUtil.compareDate(currentTime, tokenValidTime)) {
                        ...
                        response.getWriter().write(JSON.toJSONString(Result.error(Constants.CODE_401,"认证凭证已过期")));
                        return;
...
//                if (!StringUtil.equals(ip, currentIp)) {//地址不正确
                if (!StrUtil.equals(ip,currentIp)) {//地址不正确
